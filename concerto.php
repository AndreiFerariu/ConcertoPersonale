...
{   //creazione attributi
    ...
    {
        echo "inserisci codice da cercare \n";
        $idSala= readline();

        $objSala =  Sala($idSala);
        //Mostriamo tutti gli attributi dell'oggetto
        echo "Codice: " . $objSala->codice_concerto . "\n";//. serve per unire le stringhe
        echo "nome: " . $objSala->titolo_concerto . "\n";
        echo "capienza: " . $objSala->descrizione_concerto . "\n";
    }
    ...
    public function Sala($idSala)
    {
        $z = $connessione->prepare('select * from organizzazione_concerto.sale where id = :id'); //cerchiamo l'oggeto appena xcreato per l'id
        $z->bindParam(':id', $idSala, PDO::PARAM_INT);
        $z->execute();
        return $c->fetchObject(__CLASS__); //lo ritorniamo e con fetch object serve per andare a recuperare l'oggetto di tipo class 
